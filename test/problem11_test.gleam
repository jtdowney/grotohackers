...
pub fn parse_str_need_more_data_test() {
  assert problem11.parse_str(<<>>) == Error(problem11.NeedMoreData)
  assert problem11.parse_str(<<0, 0>>) == Error(problem11.NeedMoreData)
...
pub fn parse_error_test() {
  let msg = problem11.encode_message(problem11.MsgError(message: "bad"))
  let assert Ok(#(parsed, rest)) = problem11.parse_message(msg)
  assert parsed == problem11.Outbound(problem11.MsgError(message: "bad"))
  assert rest == <<>>
}

pub fn parse_ok_test() {
  let msg = problem11.encode_message(problem11.MsgOk)
  let assert Ok(#(parsed, rest)) = problem11.parse_message(msg)
  assert parsed == problem11.Outbound(problem11.MsgOk)
  assert rest == <<>>
}

pub fn parse_dial_authority_test() {
  let msg = problem11.encode_message(problem11.DialAuthority(site: 12_345))
  let assert Ok(#(parsed, rest)) = problem11.parse_message(msg)
  assert parsed == problem11.Outbound(problem11.DialAuthority(site: 12_345))
  assert rest == <<>>
}

pub fn parse_delete_policy_test() {
  let msg = problem11.encode_message(problem11.DeletePolicy(policy: 42))
  let assert Ok(#(parsed, rest)) = problem11.parse_message(msg)
  assert parsed == problem11.Outbound(problem11.DeletePolicy(policy: 42))
  assert rest == <<>>
}
...
pub fn parse_policy_result_test() {
  let msg = problem11.encode_message(problem11.PolicyResult(policy: 123))
  let assert Ok(#(parsed, rest)) = problem11.parse_message(msg)
  assert parsed == problem11.Outbound(problem11.PolicyResult(policy: 123))
  assert rest == <<>>
}

pub fn parse_need_more_data_empty_test() {
  assert problem11.parse_message(<<>>) == Error(problem11.NeedMoreData)
}
...
pub fn parse_bad_checksum_test() {
  ...
  assert problem11.parse_message(frame) == Error(problem11.BadChecksum)
}
...
pub fn parse_length_too_short_test() {
  ...
  assert problem11.parse_message(frame)
    == Error(problem11.InvalidMessage("Message length too short"))
}
...
pub fn parse_length_too_large_test() {
  ...
  assert problem11.parse_message(frame)
    == Error(problem11.InvalidMessage("Message length too large"))
}
...
pub fn spec_ok_example_test() {
  ...
  assert msg == problem11.Outbound(problem11.MsgOk)
}

pub fn spec_ok_encode_matches_test() {
  let encoded = problem11.encode_message(problem11.MsgOk)
...
pub fn spec_target_populations_example_test() {
  ...
  let assert Ok(#(msg, <<>>)) = problem11.parse_message(data)
  assert msg
    == problem11.TargetPopulations(site: 12_345, populations: [
      problem11.PopulationTarget(species: "dog", min: 1, max: 3),
      problem11.PopulationTarget(species: "rat", min: 0, max: 10),
    ])
}
...
pub fn spec_site_visit_example_test() {
  ...
  let assert Ok(#(msg, <<>>)) = problem11.parse_message(data)
  assert msg
    == problem11.SiteVisit(site: 12_345, populations: [
      problem11.PopulationObservation(species: "dog", count: 1),
      problem11.PopulationObservation(species: "rat", count: 5),
    ])
}
...
pub fn buffer_single_message_test() {
  ...
  assert messages == [problem11.Outbound(problem11.MsgOk)]
  assert remainder == <<>>
}
...
pub fn buffer_partial_message_test() {
  ...
  assert messages2 == [problem11.Outbound(problem11.MsgOk)]
  assert remainder == <<>>
}
...
pub fn validate_populations_unique_test() {
  let pops = [
    problem11.PopulationObservation(species: "dog", count: 5),
    problem11.PopulationObservation(species: "cat", count: 3),
  ]
...
pub fn validate_populations_conflicting_dupes_test() {
  let pops = [
    problem11.PopulationObservation(species: "dog", count: 5),
    problem11.PopulationObservation(species: "dog", count: 10),
  ]
...
pub fn policy_changes_in_range_delete_existing_test() {
  let targets = dict.from_list([#("dog", #(5, 20))])
  let observations = dict.from_list([#("dog", 10)])
  let policies = dict.from_list([#("dog", #(42, problem11.Conserve))])
...
pub fn policy_changes_multiple_species_test() {
  ...
  let cat_changes =
    list.filter(changes, fn(c) {
      case c {
        problem11.CreateNewPolicy(species: "cat", ..) -> True
        problem11.DeleteExistingPolicy(species: "cat", ..) -> True
        _ -> False
      }
    })
  assert cat_changes == []
